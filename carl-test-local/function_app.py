...
def codeExecute(req: func.HttpRequest, inputblob: str) -> func.HttpResponse:
    ...
    if code_text:
        ...
        with open(file_path, "w") as code_file:
            ...
            code_file.write("\n")
            code_file.write("\n")
            code_file.write("solution = Solution()")
            code_file.write("\n")
            code_file.write("tester(solution, samples, expect_outputs)")
            logging.info(f"File {file_path} created")

        python_path = "python"
        # Execute the .py file and capture the output
        try:
            result = subprocess.run([python_path, file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True)
